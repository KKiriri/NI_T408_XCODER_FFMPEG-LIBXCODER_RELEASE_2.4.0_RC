...
function parse_user_option() {
    while [ "$1" != "" ]; do
        case $1 in
            ...
            --with-sim)                 XCODER_SIM=YES;;
            --without-sim)              XCODER_SIM=NO;;
            --with-dump)                XCODER_DUMP=YES;;
            --without-dump)             XCODER_DUMP=NO;;
...
function regenerate_options() {
    ...
    if [ "$XCODER_SIM" = YES ]; then XCODER_AUTO_CONFIGURE="${XCODER_AUTO_CONFIGURE} --with-sim"; else XCODER_AUTO_CONFIGURE="${XCODER_AUTO_CONFIGURE} --without-sim"; fi
    if [ "$XCODER_DUMP" = YES ]; then XCODER_AUTO_CONFIGURE="${XCODER_AUTO_CONFIGURE} --with-dump"; else XCODER_AUTO_CONFIGURE="${XCODER_AUTO_CONFIGURE} --without-dump"; fi
...
function check_option_conflicts() {
    ...
    if [ "$XCODER_SIM" = RESERVED ]; then echo "you must specify the simulation mode, see: ./configure --help"; __check_ok=NO; fi
    if [ "$XCODER_DUMP" = RESERVED ]; then echo "you must specify the dumping, see: ./configure --help"; __check_ok=NO; fi
