...
while [ "$1" != "" ]; do
    case $1 in
        -h | --help)     echo "Usage: ./build.sh [OPTION]";
                         ...
                         echo "-s, --with-linux-custom-driver   compile with linux custom nvme driver";
        ...
        ;;
        -s | --with-linux-custom-driver)    build_linux_custom_driver=true
        ;;
...
if $target_windows; then
    ...
    if [ $RC = 0 ]; then
        make all WINDOWS=TRUE $extra_make_flags;
        RC=$?;
    else
        exit $RC;
    fi
