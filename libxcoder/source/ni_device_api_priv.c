...
{                                                                    \
  ...
  int err_rc = ni_query_status_info(ctx, type, &err_rc_info, rc); \
  ...
  if (err_rc_info.sess_err_no || ctx->rc_error_count >= NI_XCODER_FAILURES_MAX || (tmp_rc = ni_nvme_check_error_code(rc, opcode, type, hw_id, inst_id))) { \
    ni_log(NI_LOG_INFO, "Persistent failures detected, sess_err_no %u inst_err_no %u rc_error_count: %d\n", err_rc_info.sess_err_no, err_rc_info.inst_err_no, ctx->rc_error_count); \
    rc = tmp_rc; \
    LRETURN; \
  }                                                                   \
}
...
{                                                                    \
  ...
  if (err_rc_info.sess_err_no || ctx->rc_error_count >= NI_XCODER_FAILURES_MAX || ni_nvme_check_error_code(rc, opcode, type, hw_id, inst_id)) {  \
    ni_log(NI_LOG_INFO, "Terminating due to persistent failures, sess_err_no %u inst_err_no %u rc_error_count: %d\n", err_rc_info.sess_err_no, err_rc_info.inst_err_no, ctx->rc_error_count); \
    kill(getpid(), SIGTERM);                                         \
  }                                                                  \
}
...
ni_retcode_t ni_decoder_session_open(ni_session_context_t* p_ctx)
{
  ...
  for(counter = 0; counter< MAX_NUM_OF_THREADS_PER_FRAME; counter++)
  {
    ...
    {
      ni_log(NI_LOG_ERROR, "ERROR: ni_decoder_session_open failed to create decoder read threads !\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
  }
  ...
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    ...
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): alloc decoder all zero buffer failed\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
    ...
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): alloc data buffer failed\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
  ...
  if (!p_ctx->p_leftover)
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): Cannot allocate leftover buffer.\n");
    retval = NI_RETCODE_ERROR_MEM_ALOC;
  ...
  {
    ni_log(NI_LOG_TRACE, "ERROR: ni_decoder_session_open(): Cannot open dump file\n");
  }
#endif
...
ni_retcode_t ni_decoder_session_close(ni_session_context_t* p_ctx, int eos_recieved)
{
  ...
  for (counter = 0; counter< MAX_NUM_OF_THREADS_PER_FRAME; counter++)
  {
    if (p_ctx->ThreadID_decoder_read[counter])
    {
      ...
      if (ret)
      {
        ni_log(NI_LOG_TRACE, "ERROR: encoder read thread cancle wait finished failed:%d\n",ret);
  ...
  if(posix_memalign(&p_buffer, sysconf(_SC_PAGESIZE), NI_DATA_BUFFER_LEN))
  {
    ni_log(NI_LOG_ERROR, "ERROR: malloc decoder close data buffer failed\n");
    retval = NI_RETCODE_ERROR_MEM_ALOC;
    LRETURN;
  }
  memset(p_buffer, 0, NI_DATA_BUFFER_LEN);
...
int ni_decoder_session_write(ni_session_context_t* p_ctx, ni_packet_t* p_packet)
{
  ...
  {
    query_retry++;
    retval = ni_query_status_info(p_ctx, p_ctx->device_type, &inst_info,retval);
    ...
    else if (inst_info.wr_buf_avail_size < packet_size)
    {
  ...
  //TODO: How do we handle end_of_stream??
  sim_eos_flag = p_packet->end_of_stream;
#endif
  ...
  if (p_packet->p_data)
  {
    ...
    p_ctx->status = 0;
#ifdef XCODER_DUMP_ENABLED
    fwrite(p_data, packet_size, 1, p_ctx->p_dump[0]);
#endif
...
#ifdef XCODER_DUMP_ENABLED
        fwrite(p_data, write_size_bytes, 1, p_ctx->p_dump[0]);
#endif
        // reset session status after successful send
        p_ctx->status = 0;
      }
...
int ni_decoder_session_read(ni_session_context_t* p_ctx, ni_frame_t* p_frame)
{
...
#ifdef XCODER_IO_RW_ENABLED
  // To resolve low performance issue when the video resolution is low.
  // When decode, make sure that firmware receive more packets than output
  // frames. VPU always has packets to decode, so as to make full use of VPU.
  if(p_ctx->pkt_num - p_ctx->frame_num < 10 && !p_ctx->ready_to_close)
  {
      retval = NI_RETCODE_SUCCESS;
      LRETURN;
  }
  ...
  while (1)
  {
    ...
    {
#ifdef XCODER_IO_RW_ENABLED
      ni_log(NI_LOG_ERROR, "Error dec read query fail rc %d retry %d\n",
                     retval, query_retry);
...
ni_retcode_t ni_encoder_session_open(ni_session_context_t* p_ctx)
{
  ...
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): alloc dts_queue_mutex fail!, return\n");
    retval = NI_RETCODE_ERROR_MEM_ALOC;
    LRETURN;
  }
#ifndef XCODER_IO_RW_ENABLED //No need background RW if XCODER_IO_RW_ENABLED
  ...
  for(counter = 0; counter<MAX_NUM_OF_THREADS_PER_FRAME; counter++)
  {
    ...
    {
      ni_log(NI_LOG_ERROR, "ERROR: ni_encoder_session_open failed to create encoder read threads !\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
    ...
    {
      ni_log(NI_LOG_ERROR, "ERROR: ni_encoder_session_open failed to create encoder write threads !\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
  }
  ...
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    ...
    if(posix_memalign(&p_ctx->p_all_zero_buf, sysconf(_SC_PAGESIZE), NI_DATA_BUFFER_LEN))
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_encoder_session_open(): alloc all zero buffer failed\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
    memset(p_ctx->p_all_zero_buf, 0, NI_DATA_BUFFER_LEN);
    ...
    if(posix_memalign(&p_buffer, sysconf(_SC_PAGESIZE), NI_DATA_BUFFER_LEN))
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_encoder_session_open(): alloc data buffer failed\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
    memset(p_buffer, 0, NI_DATA_BUFFER_LEN);
    ...
    p_ctx->session_id = ((ni_get_session_id_t *)p_buffer)->session_id;
    ni_log(NI_LOG_TRACE, "Got open session ID:0x%x\n",p_ctx->session_id);
    if (NI_INVALID_SESSION_ID == p_ctx->session_id)
    {
  ...
  if (NI_RETCODE_SUCCESS != retval)
  {
    ni_log(NI_LOG_TRACE, "ERROR: calling ni_config_instance_set_encoder_params(): p_ctx->device_handle=%" PRIx64 ", p_ctx->hw_id=%d, p_ctx->session_id=%d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
    ni_encoder_session_close(p_ctx, 0);
    retval = NI_RETCODE_ERROR_INVALID_SESSION;
    LRETURN;
  }
  ...
  snprintf(dump_file, sizeof dump_file, "%s%d%s", "encoder_in_id", instance_id, ".yuv");
  p_ctx->p_dump[0] = fopen(dump_file, "wb");
  ni_log(NI_LOG_TRACE, "dump_file = %s\n", dump_file);

  snprintf(dump_file, sizeof dump_file, "%s%d%s", "encoder_out_id", instance_id, ".265");
  p_ctx->p_dump[1] = fopen(dump_file, "wb");
  ni_log(NI_LOG_TRACE, "dump_file = %s\n", dump_file);

  if (!p_ctx->p_dump[0] || !p_ctx->p_dump[1])
  {
    ni_log(NI_LOG_TRACE, "ERROR: Cannot open dump file\n");
  }
#endif
  ...
  while (1)
  {
    ...
    {
      ni_log(NI_LOG_INFO, "ERROR: session error %u or VPU_RSRC_INSUFFICIENT\n",
...
ni_retcode_t ni_encoder_session_close(ni_session_context_t* p_ctx, int eos_recieved)
{
  ...
  for (counter = 0; counter< MAX_NUM_OF_THREADS_PER_FRAME; counter++)
  {
    if (p_ctx->ThreadID_encoder_write[counter])
    {
      ...
      if (ret)
      {
        ni_log(NI_LOG_TRACE, "ERROR: encoder write thread cancle wait finished failed:%d\n",ret);
  ...
  if(posix_memalign(&p_buffer, sysconf(_SC_PAGESIZE), NI_DATA_BUFFER_LEN))
  {
    ni_log(NI_LOG_ERROR, "Error ni_encoder_session_close(): alloc data buffer failed\n");
    retval = NI_RETCODE_ERROR_MEM_ALOC;
    LRETURN;
  }
  memset(p_buffer, 0, NI_DATA_BUFFER_LEN);
...
int ni_encoder_session_write(ni_session_context_t* p_ctx, ni_frame_t* p_frame)
{
  ...
  while (1)
  {
    ...
    else if (inst_info.wr_buf_avail_size < frame_size_bytes)
    {
      ni_log(NI_LOG_TRACE, "Warning enc write query fail rc %d or available "
             "buf size %u < frame size %u !\n", retval,
      ...
      if (send_count >= 2000)
      {
        ni_log(NI_LOG_ERROR, "ERROR enc query buf info exceeding max retries: "
                       "%d, ", NI_MAX_TX_RETRIES);
        p_ctx->status = NI_RETCODE_NVME_SC_WRITE_BUFFER_FULL;
        retval = NI_RETCODE_SUCCESS;
        LRETURN;
  ...
  p_meta->force_headers = 0; // p_frame->force_headers not implemented yet
  ...
  ni_log(NI_LOG_TRACE, "%s(): %d.%d p_ctx->frame_num=%lu, p_frame->start_of_stream=%u, p_frame->end_of_stream=%u, p_frame->video_width=%u, p_frame->video_height=%u, pts=0x%08x 0x%08x, dts=0x%08x 0x%08x, sei_len=%u, roi size=%u avg_qp=%u reconf_len=%u force_pic_qp=%u\n",
                 __FUNCTION__,
                 p_ctx->hw_id, p_ctx->session_id, p_ctx->frame_num, p_frame->start_of_stream, p_frame->end_of_stream, p_frame->video_width, p_frame->video_height, (uint32_t)((p_frame->pts >> 32) & 0xFFFFFFFF),
                 (uint32_t)(p_frame->pts & 0xFFFFFFFF), (uint32_t)((p_frame->dts >> 32) & 0xFFFFFFFF), (uint32_t)(p_frame->dts & 0xFFFFFFFF),
                 p_meta->frame_sei_data_size, p_meta->frame_roi_map_size,
...
#ifdef XCODER_DUMP_ENABLED
      fwrite(p_frame->p_buffer, frame_size_bytes, 1, p_ctx->p_dump[0]);
#endif
      p_ctx->frame_num++;
      size = frame_size_bytes;
...
int ni_encoder_session_read(ni_session_context_t* p_ctx, ni_packet_t* p_packet)
{
  ...
  while (1)
  {
    ...
    ni_log(NI_LOG_TRACE, "Info enc read query rc %d, available buf size %u, "
           "frame_num=%"PRIu64", pkt_num=%"PRIu64"\n", retval,
           inst_info.rd_buf_avail_size, p_ctx->frame_num, p_ctx->pkt_num);
    ...
    else if (0 == inst_info.rd_buf_avail_size)
    {
      ...
      if ((((ni_encoder_params_t *)p_ctx->p_session_config)->low_delay_mode || (NI_RETCODE_NVME_SC_WRITE_BUFFER_FULL == p_ctx->status)) &&
          ! p_packet->end_of_stream && p_ctx->frame_num >= p_ctx->pkt_num)
      {
        ni_log(NI_LOG_TRACE, "Encoder low latency mode, eos not sent, frame_num "
  ...
  {
    actual_read_size = ((actual_read_size / NI_MEM_PAGE_ALIGNMENT) * NI_MEM_PAGE_ALIGNMENT) + NI_MEM_PAGE_ALIGNMENT;
  }
  ...
  if (retval < 0)
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_encoder_session_read(): nvme command failed\n");
    retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
    LRETURN;
  }
...
#ifdef XCODER_DUMP_ENABLED
  fwrite(p_packet->data, p_packet->data_len, 1, p_ctx->p_dump[1]);
#endif
...
#ifdef XCODER_SIM_ENABLED
  rx_size = p_frame->data_len[0] + p_frame->data_len[1] + p_frame->data_len[2];
  p_frame->end_of_stream = sim_eos_flag;
  sim_run_count++;
  if (sim_run_count < 2) //100
  {
    rx_size = 0;
  }
  else
  ...
  ni_encoder_params_t* p_src)
{
  ...
  if (p_t408->cu_size_mode != p_enc->cu_size_mode)
  {
    p_t408->cu_size_mode = p_enc->cu_size_mode;
  }

  if (p_t408->max_num_merge != p_enc->max_num_merge)
  {
...
void ni_params_print(ni_encoder_params_t* const p_encoder_params)
{
  ...
  ni_log(NI_LOG_TRACE, "cu_size_mode=%d\n", p_enc->cu_size_mode);
  ni_log(NI_LOG_TRACE, "max_num_merge=%d\n", p_enc->max_num_merge);
...
void *ni_session_keep_alive_thread(void *arguments)
{
  ...
  {
    retval = ni_send_session_keep_alive(args->session_id, args->device_handle, args->thread_event_handle, args->p_buffer);
    if (NI_RETCODE_SUCCESS != retval)
    {
       ni_log(NI_LOG_TRACE, "keep_alive_thread abormal closed:%d\n", retval);
#ifdef _WIN32
       return NULL;
#else
       pthread_exit (NULL);
    ...
    if (args->close_thread)
    {
      ni_log(NI_LOG_TRACE, "keep_alive_thread closed\n");
      break;
    }
  }
  return NULL;
