...
void setup_signal_handler(void)
{
  ...
  {
    fprintf(stderr, "Error signal handler setup errno: %d\n", errno);
  }
}
                ...
                ni_session_context_t *sessionCtxt)
{
  ...
  if (module_type == NI_DEVICE_TYPE_DECODER)
  {
    ...
    if(!dev_ctxt_arr)
    {
      fprintf(stderr, "ERROR: Failed to allocate memory for ni_rsrc_mon print_perf()");
      return;
    }
  ...
  for (i = 0; i < module_count; i++)
  {
    ...
    if (p_device_context)
    {
      ...
      if (NI_INVALID_EVENT_HANDLE == sessionCtxt->event_handle)
      {
        ...
        fprintf(stderr, "Error create envet:%d\n", GetLastError());
        continue;
      }
#endif
#endif
...
int main(int argc, char *argv[])
{
  ...
  // arg handling
  while ((opt = getopt(argc, argv, "hrn:o:t:l:")) != -1)
  {
    switch (opt)
    {
    case 'o':
