...
void ni_rsrc_get_one_device_info(ni_device_info_t* p_device_info)
{
  ...
  if (NULL == p_device_info->lock)
  {
    mutex_handle = CreateMutex(
                                    NULL,            // default security attributes
                                    TRUE,            // initially owned
                                    (LPCSTR)lck_name  // unnamed mutex
                                );
    ...
    if (NULL == mutex_handle)
    {
      ...
      {
        ni_log(NI_LOG_INFO, "CreateMutex %s failed: error: %d\n", lck_name, GetLastError());
        LRETURN;
      }
    }

    p_device_info->lock = mutex_handle;
  }
  ...
  {
    rc = GetLastError();
    ni_log(NI_LOG_INFO, "ERROR: CreateFileMapping returned (%d). for %s\n", rc, shm_name);
    LRETURN;
  }
  else
  {
...
void ni_rsrc_get_one_device_info (ni_device_info_t * p_device_info)
{
  ...
  if (service == NULL)
  {
    perror("ni_rsrc_get_one_device_info Error service ..");
    LRETURN;
  }
  shm_fd = service->getFd(param);
  if (shm_fd < 0)
  ...
  {
    ni_log(NI_LOG_ERROR, "Error mmap ...");
    error = NI_RETCODE_FAILURE;
    LRETURN;
  }
  ...
  if (lock < 0)
  {
    ni_log(NI_LOG_INFO, "Error open lock file ... %s", lck_name) ;
    error = NI_RETCODE_FAILURE;
    LRETURN;
  }
