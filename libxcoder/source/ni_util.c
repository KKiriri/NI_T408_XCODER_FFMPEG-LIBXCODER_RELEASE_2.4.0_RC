...
uint32_t ni_get_kernel_max_io_size(const char * p_dev)
{
  ...
  if(!p_file)
  {
    ni_log(NI_LOG_TRACE, "file_name  failed to open: %s\n",file_name);
    LRETURN;
  }
  ...
  if(EOF == err)
  {  
    ni_log(NI_LOG_TRACE, "fscanf failed on: %s max_segments\n",file_name);
    LRETURN;
  }
                                          ...
                                          int height_align, int factor)
{
  ...
  {
    ni_log(NI_LOG_TRACE, "Error alloc for dec fme buf pool\n");
    return -1;
  }
...
uint32_t remove_substring_pattern(char *main_str, const char *pattern)
{
  ...
  for (i = 0; i < strlen(main_str) && !pattern_matched; i++)
  {
    ...
    {
      matched_chr = 0;
      memset(char_match_pattern, 0, 10);
      ...
      {
        memcpy(char_match_pattern, digit_match_pattern, strlen(digit_match_pattern));
      }
      else
      {
      ...
      if (pattern[j+1] == '+')
      {
        while (strchr(char_match_pattern, (int) main_str[i + match_length]))
        {
          match_length++;
          matched_chr = 1;
        }
      ...
      else if (strchr(char_match_pattern, (int) main_str[i + match_length]))
      {
        match_length++;
        matched_chr = 1;
      }
...
ni_retcode_t ni_find_blk_name(const char *p_dev, char *p_out_buf, int out_buf_len)
{
  ...
  snprintf(cmd, sizeof(cmd) - 1, "ls /dev/block/ | grep %s", &p_dev[5]); // p_dev[5] is p_dev without '/dev/'
#else
...
int threadpool_auto_add_task_thread(threadpool_t *pool, void *(*run)(void *arg), void *arg, int newThread)
{
  ...
  if (!newtask)
  {
    ni_log(NI_LOG_ERROR, "ERROR: threadpool_auto_add_task_thread Failed to allocate memory\n");
    return -1;
  }
  newtask->run = run;
  newtask->arg = arg;
  ...
  else
  {
    if (pool->counter < pool->max_threads)
    {
      if (newThread == 1)
      {
        ...
        if (ret)
	    {
          ni_log(NI_LOG_ERROR, "ERROR: threadpool_auto_add_task_thread pthread_create failed : %d\n", ret);
          return -1;
        }
        pool->counter++;
      }
...
int threadpool_add_task(threadpool_t *pool, void *(*run)(void *arg), void *arg)
{
  ...
  if (!newtask)
  {
    ni_log(NI_LOG_ERROR, "ERROR: threadpool_add_task Failed to allocate memory\n");
    return -1;
  }
  newtask->run = run;
  newtask->arg = arg;
  ...
  else if (pool->counter < pool->max_threads)
  {
    ...
    if (ret)
	{
      ni_log(NI_LOG_ERROR, "ERROR: threadpool_add_task pthread_create failed : %d\n", ret);
      return -1;
    }
    pool->counter++;
  }
...
ni_lat_meas_q_t * ni_lat_meas_q_create(unsigned capacity)
{
    ...
    if(!queue)
    {
      perror("ERROR: Failed to allocate memory for lat_meas-queue queue");
      return NULL;
    }
    ...
    if(!queue->array)
    {
      perror("ERROR: Failed to allocate memory for lat_meas_queue queue->array");
      return NULL;
    }
    return queue;
}
