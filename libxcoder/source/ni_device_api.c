 ...
 *******************************************************************************/
 ni_session_context_t * ni_device_session_context_alloc_init(void)
 {
   ...
   if (!p_ctx)
   {
     ni_log(NI_LOG_TRACE, "ERROR: Failed to allocate memory for session context\n");
   }
   else 
   {
...
ni_event_handle_t ni_create_event()
{
  ...
  {
    retval = GetLastError();
    ni_log(NI_LOG_ERROR, "ERROR: create event error %d\n",retval);
...
void ni_close_event(ni_event_handle_t event_handle)
{
  ...
  if (FALSE == retval)
  {
    ...
    ni_log(NI_LOG_TRACE, "Warning ni_device_close(): closing device device_handle %p failed, error: %d\n", event_handle, error);
  }
  else
  {
...
ni_device_handle_t ni_device_open(const char * p_dev, uint32_t * p_max_io_size_out)
{
  ...
  *p_max_io_size_out =  0; 
  ...
  if (fd < 0)
  {
    ni_log(NI_LOG_TRACE, "ERROR: open() failed on %s\n", p_dev);
...
void ni_device_close(ni_device_handle_t device_handle)
{
  ...
  if (FALSE == retval)
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_device_close() cancel io failed %d\n", GetLastError());
  }
  ...
  {
    DWORD error = GetLastError();
    ni_log(NI_LOG_TRACE, "ERROR: ni_device_close(): closing device device_handle %p failed, error: %d\n", device_handle, error);
  }
  else
  {
    ni_log(NI_LOG_TRACE, "ni_device_close(): device %p closed successfuly\n", device_handle);
...
ni_retcode_t ni_device_capability_query(ni_device_handle_t device_handle, ni_device_capability_t* p_cap)
{
  ...
  {
    ni_log(NI_LOG_TRACE, "ERROR: ni_device_capability_query(): Cannot allocate buffer.\n");
...
ni_retcode_t ni_device_session_open( ni_session_context_t *p_ctx, ni_device_type_t device_type )
{
    ...
    else if ((0 == strcmp(p_ctx->dev_xcoder, BEST_MODEL_LOAD_STR)) || (0 == strcmp(p_ctx->dev_xcoder, BEST_DEVICE_INST_STR)))
    {
      ...
      for (i = 0; i < num_coders; i++)
      {
        ...
        else
        {
          ...
          if ((i == 0) || (p_dev_info->active_num_inst < least_instance) ||
  ...
  {
    ni_log(NI_LOG_TRACE, "ERROR: thread_args allocation failed!\n");
  ...
  {
    ni_log(NI_LOG_TRACE, "ERROR: failed to create keep alive thread\n");
...
ni_retcode_t ni_frame_buffer_alloc(ni_frame_t* p_frame, int video_width, int video_height, int alignment, int metadata_flag, int factor)
{
  ...
  if (p_frame->buffer_size != buffer_size)
  {
    ...
    {
      ni_log(NI_LOG_TRACE, "ERROR: Cannot allocate p_frame buffer.\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
#else
...
ni_retcode_t ni_packet_buffer_alloc(ni_packet_t* p_packet, int packet_size)
{
  ...
  if (posix_memalign(&p_buffer, sysconf(_SC_PAGESIZE), buffer_size))
  {
    ni_log(NI_LOG_TRACE, "ERROR: ni_packet_buffer_alloc(): Cannot allocate p_frame buffer.\n");
    retval = NI_RETCODE_ERROR_MEM_ALOC;
    LRETURN;
  }
#else
...
ni_retcode_t ni_parse_reconf_file(const char* reconf_file, int hash_map[100][10])
{
  ...
  if (!reconf)
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_parse_reconf_file(): Cannot open reconfig_file: %s\n", reconf_file);
    return NI_RETCODE_FAILURE;
  }
