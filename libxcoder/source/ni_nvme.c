...
#ifdef XCODER_IO_RW_ENABLED
  if (posix_memalign(&p_buffer, sysconf(_SC_PAGESIZE), data_len))
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_nvme_enumerate_devices(): alloc buffer failed\n");
    return NI_RETCODE_ERROR_MEM_ALOC;
  }
...
{
  uint64_t lba_low18 = (lba & 0x3FFFF);
  if ((lba >>NI_INSTANCE_OFFSET) & 0x1)
  {
    ni_log(NI_LOG_ERROR, "encoder instance:%lu\n",((lba >>NI_SESSION_ID_OFFSET)&0x7FU));
    if (lba_low18 > WR_OFFSET_IN_4K)
    {
      ni_log(NI_LOG_ERROR, "encoder send frame failed\n");
    }
    else if(lba_low18 > RD_OFFSET_IN_4K)
    {
      ni_log(NI_LOG_ERROR, "encoder receive packet failed\n");
    }
    else
    {
      ni_log(NI_LOG_ERROR, "encoder ctrl command failed: op-0x%x, sub-0x%x, subtype-0x%x\n", \
                      (uint32_t)(((lba_low18 - START_OFFSET_IN_4K)>>NI_OP_BIT_OFFSET)+0xD0), \
                      (uint32_t)((lba_low18>>NI_SUB_BIT_OFFSET) & 0xF), \
                      (uint32_t)(lba_low18 & 0xF));
    }
  }
  else
  {
    ni_log(NI_LOG_ERROR, "decoder instance:%lu\n",((lba >>NI_SESSION_ID_OFFSET)&0x7FU));
    if (lba_low18 > WR_OFFSET_IN_4K)
    {
    ...
    {
      ni_log(NI_LOG_ERROR, "decoder ctrl command failed: op-0x%x, sub-0x%x, subtype-0x%x\n", \
                      (uint32_t)(((lba_low18 - START_OFFSET_IN_4K)>>NI_OP_BIT_OFFSET)+0xD0), \
                      (uint32_t)((lba_low18>>NI_SUB_BIT_OFFSET) & 0xF), \
                      (uint32_t)(lba_low18 & 0xF));
    }
...
int32_t ni_nvme_send_read_cmd(ni_device_handle_t handle, ni_event_handle_t event_handle, void *p_data, uint32_t data_len, uint32_t lba)
{
  ...
  if (rc == FALSE)
  {
    ...
    if(rc == ERROR_IO_PENDING)
    {
      ...
      if (rc == WAIT_OBJECT_0)
      {
        ...
        {
          rc = GetLastError();
          ni_log(NI_LOG_ERROR, "ni_nvme_send_read_cmd Error: %d\n",rc);
          ni_parse_lba(lba);
      ...
      {
        rc = GetLastError();
        ni_log(NI_LOG_ERROR, "ni_nvme_send_read_cmd() wait Other Error:%d\n",rc);
        ni_parse_lba(lba);
        return NI_RETCODE_ERROR_NVME_CMD_FAILED;
      }
    }
  ...
  if ((rc < 0) || (rc != data_len))
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_nvme_send_read_cmd failed, lba=0x%x, len=%d, rc=%d, error=%d, %s\n", (lba<<3), data_len, rc, errno, strerror(errno));
    ni_parse_lba(lba);
...
int32_t ni_nvme_send_write_cmd(ni_device_handle_t handle, ni_event_handle_t event_handle, void *p_data, uint32_t data_len, uint32_t lba)
{
  ...
  if (rc == FALSE)
  {
    ...
    if(rc == ERROR_IO_PENDING)
    {
      ...
      if (rc == WAIT_OBJECT_0)
      {
        ...
        {
          rc = GetLastError();
          ni_log(NI_LOG_ERROR, "ni_nvme_send_write_cmd Error: %d\n",rc);
          ni_parse_lba(lba);
          return NI_RETCODE_ERROR_NVME_CMD_FAILED;
        }
        else
      ...
      {
        rc = GetLastError();
        ni_log(NI_LOG_ERROR, "ni_nvme_send_write_cmd() wait Other Error:%d\n",rc);        
        ni_parse_lba(lba);
        return NI_RETCODE_ERROR_NVME_CMD_FAILED;
      }
    }
  ...
  if ((rc < 0) || (rc != data_len))
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_nvme_send_write_cmd failed, lba=0x%x, len=%d, rc=%d, error=%d, %s\n", (lba<<3), data_len, rc, errno, strerror(errno));
    ni_parse_lba(lba);
    rc = NI_RETCODE_ERROR_NVME_CMD_FAILED;
  }
