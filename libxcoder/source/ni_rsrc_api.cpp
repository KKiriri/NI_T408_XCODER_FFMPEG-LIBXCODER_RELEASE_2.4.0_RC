...
ni_device_pool_t* ni_rsrc_get_device_pool(void)
{
  ...
  {
    ni_log(NI_LOG_ERROR, "Error malloc ..");
  }
  else
  {
...
#define SLEEPLOOP 3
  ...
  ReleaseMutex(lock);
  ReleaseMutex(lock_decoder);
  ReleaseMutex(lock_encoder);
...
ni_retcode_t ni_rsrc_refresh(int should_match_rev)
{
  ...
  if (0 == xcoder_dev_count)
  {
    ...
    if (NI_RETCODE_SUCCESS == ni_rsrc_list_devices(
          NI_DEVICE_TYPE_DECODER, saved_coders.decoders,
          &(saved_coders.decoders_cnt)))
    {
...
ni_device_context_t* ni_rsrc_get_device_context(ni_device_type_t device_type, int guid)
{
  ...
  if (NULL == mutex_handle)
  {
    ni_log(NI_LOG_INFO, "CreateMutex error: %d\n", GetLastError());
                                  ...
                                  int    max_handles)
{
  ...
  {
    ni_log(NI_LOG_INFO, "Error: failed to open directory %s\n", dir_name);
    return 0;
  }
...
ni_device_pool_t* ni_rsrc_get_device_pool(void)
{
  ...
  if (0 != access(workDir,0))
  {
    ...
    {
      perror("Error create /dev/shm folder...");
      return NULL;
    }
  }
#endif
  ...
  if (lock < 0)
  {
    ni_log(NI_LOG_ERROR, "Error open CODERS_LCK_NAME...");
    return NULL;
  }
  lockf(lock, F_LOCK, 0);
  ...
  if (service == NULL)
  {
    perror("ni_rsrc_get_device_pool Error service ..");
    LRETURN;
  }
  ...
  if (shm_fd < 0)
  {
    ni_log(NI_LOG_ERROR, "Error shm_open SHM_CODERS ..");
    LRETURN;
  }
  ...
  {
    ni_log(NI_LOG_ERROR, "Error mmap ni_device_queue_t ..");
    LRETURN;
  }
...
#define SLEEPLOOP 3
  ...
  ni_lock_handle_t lock, locken, lockde;
  ni_device_handle_t dev_handle = NI_INVALID_DEVICE_HANDLE;
  ni_device_capability_t device_capabilites;
  ni_retcode_t rc;
  ...
  if (service == NULL)
  {
	perror("ni_rsrc_init 000 Error service ..");
	return 1;
  }
  ...
  {
    printf("NI resource init'd already ..\n");
	return 0;
  }
#else
...
read_dev_files:
  ...
  if (service == NULL)
  {
	perror("ni_rsrc_init 111 Error service ..");
	return 1;
  }
  shm_fd = service->getFd(param);
  if (shm_fd < 0)
  ...
  if (shm_fd < 0)
  {
    ni_log(NI_LOG_ERROR, "Error shm_open ..");
    return 1;
  }
  ...
  if (MAP_FAILED == p_device_queue)
  {
    ni_log(NI_LOG_INFO, "Error mmap ni_device_queue_t ... trouble ..\n");
    //TODO: Return without closing the shared memory file descriptor flose(shm_fd)!!!
    return 1;
  }
  ...
  if (lock < 0)
  {
    ni_log(NI_LOG_ERROR, "Error open lock file ....");
    //TODO: Return without umappint the shared memory munmap(p_device_queue)!!!
    return 1;
  }
...
ni_device_context_t* ni_rsrc_get_device_context(ni_device_type_t device_type, int guid)
{
  ...
  if (lock < 0)
  {
    ni_log(NI_LOG_INFO, "Error open lock file ..... %s %d \n",lck_name, guid);
    return NULL;
  }
  ...
  if (service == NULL)
  {
    perror("ni_rsrc_get_device_context Error service ..");
    LRETURN;
  }
  shm_fd = service->getFd(param);
  if (shm_fd < 0)
  ...
  if (MAP_FAILED == p_device_queue)
  {
    ni_log(NI_LOG_ERROR, "Error mmap ni_rsrc_get_device_context ...");
    LRETURN;
  }
...
int ni_rsrc_lock_and_open(int device_type, ni_lock_handle_t* lock)
{
  ...
  if (NI_DEVICE_TYPE_DECODER == device_type)
  {
    ...
    *lock = CreateMutex(
    ...
    if (NULL == *lock)
    {
      ni_log(NI_LOG_TRACE, "nirsrc_lock_and_open() CreateMutex %s failed: %d\n", CODERS_RETRY_DELCK_NAME, GetLastError());
  ...
  do
  {
    ...
    {
        ni_log(NI_LOG_TRACE, "ERROR: ni_rsrc_lock_and_open() failed to obtain mutex: %d, \n", GetLastError());
    }
#else
    ...
    if (status != 0)
    {
      ...
      if (count > MAX_LOCK_RETRY)
      {
        ni_log(NI_LOG_TRACE, "Can not put down the lock after 6s");
        return NI_RETCODE_ERROR_LOCK_DOWN_DEVICE;
      }
    }
  }
